...
This option uses the committed `do-app.yaml` file in the repo root to deploy all 10 services. DigitalOcean builds the Dockerfiles, assigns internal DNS names for gRPC communication, and scales the apps.

1. Ensure the `doctl` CLI is installed and authenticated.
2. Update the placeholder values in `do-app.yaml` with your real production secrets and connection strings.
3. Deploy the spec:
   ```bash
   doctl apps create --spec do-app.yaml
   ```

The manifest includes the public gateway route, the mixed HTTP/gRPC ports, and the production env vars for CockroachDB, Redis, ClickHouse, SafeHaven, and admin auth.
...
```bash
migrate -path migrations -database "postgresql://moses:NDnjNOsayOaJ4lqkSdNppA@general-16173.jxf.gcp-europe-west3.cockroachlabs.cloud:26257/instant-tf?sslmode=verify-full" up
```
