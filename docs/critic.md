...
- **Critique:** If the service crashes *after* the SafeHaven API call but *before* the DB update, the system is left in an inconsistent state (`PROCESSING` in DB, but possibly successful in SafeHaven).
