...
func main() {
	...
	// Parse listen port
	parts := strings.Split(cfg.GRPCReconciliationAddr, ":")
	...
	recWorker := worker.NewReconciliationWorker(pool, shClient, logger)

	// Start background cron tickers
	recWorker.StartCronJobs(context.Background())

	grpcServer := grpc.NewServer()
	pb.RegisterReconciliationServiceServer(grpcServer, recWorker)
	...
	go func() {
		...
		<-sigChan
		logger.Info("shutting down gRPC reconciliation server...")
		grpcServer.GracefulStop()
	}()

	logger.Info("starting gRPC reconciliation service", zap.String("addr", cfg.GRPCReconciliationAddr))
